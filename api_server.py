...
class ServiterAPIHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == "/metrics":
            body = self.runtime.observability.prometheus().encode("utf-8")
        ...
        elif parsed.path == "/certificates/latest":
            self._json(200, self.runtime.certificates.latest())
        else:
    ...
    def do_POST(self):
        ...
        elif parsed.path == "/certificates/import":
            csv_path = body.get("csv_path")
            if not csv_path:
                self._json(400, {"error": "csv_path required"})
            else:
                self._json(200, self.runtime.certificates.import_csv(csv_path))
